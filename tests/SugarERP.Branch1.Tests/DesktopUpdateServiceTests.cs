...
public sealed class DesktopUpdateServiceTests
{
    ...
    public async Task DetectsNewerCompatibleVersion()
    {
        var handler = new StubHandler(request => request.RequestUri!.AbsolutePath.EndsWith("/current")
            ? Json("""{"profile":"KITCHEN","version":"1.4.0","filename":"Sugar-Kitchen-1.4.0.exe","publishedAt":"2026-09-18T00:00:00Z","sha256":"aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa","required":false,"signed":true,"downloadUrl":"current/download"}""")
    ...
    {
        var handler = new StubHandler(_ => Json("""{"profile":"BRANCH_TYPE_2","version":"2.0.0","filename":"x.exe","publishedAt":"2026-09-18T00:00:00Z","sha256":"aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa","required":false,"signed":true,"downloadUrl":"current/download"}"""));
        var service = new DesktopUpdateService(new HttpClient(handler), new(DesktopApplicationType.Kitchen,
    ...
    public async Task DownloadsAndVerifiesHash()
    {
        ...
        var release = new DesktopReleaseManifest("BRANCH_TYPE_1", "1.1.0", "Sugar-Branch-Type-1-1.1.0.exe",
