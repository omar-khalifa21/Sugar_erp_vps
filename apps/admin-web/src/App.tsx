...
function KitchenPage({ data, onLoad, token }: { data: AdminData; onLoad: () => Promise<KitchenOverview>; token: string }) {
  ...
  const [message, setMessage] = useState('');
  useEffect(() => { let mounted = true; const refresh = () => onLoad().then((result) => { if (mounted) setOverview(result); }).catch((caught) => { if (mounted) setMessage(friendlyError(caught)); }); void refresh(); const timer = window.setInterval(() => void refresh(), 10_000); return () => { mounted = false; window.clearInterval(timer); }; }, [onLoad]);
...
function OperationsPage({ data, onIssueEnrollmentToken, onDisableDevice, token }: { data: AdminData; token: string; onIssueEnrollmentToken: (siteId: string) => Promise<{ id: string; token: string; siteId: string; profile: SiteType; expiresAt: string }>; onDisableDevice: (id: string) => Promise<Device> }) {
  ...
  useEffect(() => { void api.reports(token).then(setReports).catch(() => setReports([])); }, [token]);
  useEffect(() => () => downloadController.current?.abort(), []);
  const latestVersions: Partial<Record<SiteType, string>> = {
    BRANCH_TYPE_1: branchOneRelease?.version,
    BRANCH_TYPE_2: branchTwoRelease?.version,
    KITCHEN: kitchenRelease?.version,
  };
  return <div className="page-stack">
    ...
    <section className="panel installers">{message && <div className="form-error" role="alert">{message}</div>}<PanelHeading title="تنزيل تطبيقات نقاط التشغيل" subtitle="مثبتات Windows ذاتية الاحتواء؛ تحقق من SHA-256 بعد التنزيل" /><div className="installer-grid">{([{ label: 'فرع نوع ١ · مكتبي', release: branchOneRelease, variant: 'desktop' }, { label: 'فرع نوع ١ · لمس', release: branchOneTouchRelease, variant: 'touch' }, { label: 'المطبخ · كمبيوتر مكتبي', release: kitchenRelease, variant: 'kitchen' }, { label: siteTypeLabels.BRANCH_TYPE_2, release: branchTwoRelease, variant: 'branch2' }] as const).map(({ label, release, variant }) => <article key={label}><div className="installer-icon"><Download /></div><div><h3>{label}</h3><p>{release ? `Windows x64 · ${release.version} · ${formatDate(release.publishedAt)}` : 'لا يوجد إصدار منشور حالياً'}</p></div><button className="secondary-button" disabled={!release || !variant || downloading !== null} onClick={async () => { if (!variant) return; const controller = new AbortController(); downloadController.current = controller; setDownloading(variant); setDownloadProgress(0); setMessage(''); try { if (variant === 'kitchen') await api.downloadKitchen(token, setDownloadProgress, controller.signal); else if (variant === 'branch2') await api.downloadBranchTwo(token, setDownloadProgress, controller.signal); else await api.downloadBranchOne(token, variant, setDownloadProgress, controller.signal); } catch (caught) { if (!(caught instanceof DOMException && caught.name === 'AbortError')) setMessage(friendlyError(caught)); } finally { if (downloadController.current === controller) downloadController.current = null; setDownloading(null); } }}><Download size={17} /> {downloading === variant ? `جارٍ التنزيل ${downloadProgress}%` : release ? 'تنزيل المثبت' : 'غير متاح'}</button>{downloading === variant && <button className="cancel-download-button" onClick={() => downloadController.current?.abort()}><X size={17} /> إلغاء التنزيل</button>}{release && <small className="ltr">SHA-256: {release.sha256}</small>}</article>)}</div></section>
  </div>;
}
