...
- No licence screen. The former blocking device-enrollment screen was removed; secure one-time VPS device linking remains in Settings. It is not a paid licence. The application is production-only and requires a real HTTPS `/api/v1` address plus an admin-issued enrollment token before it can operate and synchronize.
...
- Background loop was added to the Branch Type 1 app; it runs every 15 seconds, with cancellation on exit. A semaphore serializes background and manual full sync. Existing retry/backoff and sequence ordering remain in the sync service. Manual sync is still available.
