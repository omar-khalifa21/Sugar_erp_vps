...
@Public()
@Controller('releases/branch-type-2')
export class BranchTwoReleasesController {
  private readonly directory = resolve(process.env.RELEASES_DIR || '/var/lib/sugar-erp/releases', 'branch-type-2');
  private async current() {
    try {
      ...
      if (manifest.channel !== 'production' || !/^Sugar-Branch2-[A-Za-z0-9._-]+\.exe$/.test(manifest.filename) || !/^[a-f0-9]{64}$/i.test(manifest.sha256) || !/^[A-Za-z0-9._-]+$/.test(manifest.version)) throw new Error();
      const file = resolve(this.directory, manifest.filename); if (!file.startsWith(this.directory + sep)) throw new Error();
