[CmdletBinding()] param([string]$Version='0.1.0', [uri]$ApiBaseUrl='https://ascendyz.xyz/api/v1', [switch]$Production)
...
& "$PSScriptRoot\..\shared\Sign-Artifact.ps1" -Path "$stage\SugarERP.Kitchen.Desktop.exe" -Production:$Production
...
& "$PSScriptRoot\..\shared\Sign-Artifact.ps1" -Path "$out\Sugar-Kitchen-$Version.exe" -Production:$Production
if($Production){ & "$PSScriptRoot\..\shared\Write-ReleaseManifest.ps1" -Artifact "$out\Sugar-Kitchen-$Version.exe" -Version $Version -Output "$out\current.json" -ReleaseNotes 'Kitchen recipes, ingredient receiving, waste, counts, and immutable Excel reports' }
