# Implementation agent prompt: VPS / Admin
...
**First action: inspect the prepared repository, then ask once for the final project/repository name, GitHub owner, and whether a remote already exists. Local planning and implementation may continue while remote push and deployment remain approval-gated.**

## Required execution order

This is the first of four implementation chats. Before building the whole Admin product, create the shared repository foundation and freeze `docs/contracts/CONTRACT_V1.md`. That contract must contain versioned OpenAPI, JSON schemas/examples, error codes, event envelopes, state machines, permissions, database conventions, compatibility rules, and test fixtures sufficient for all desktop chats.

Commit the foundation as an isolated checkpoint and produce `docs/handoffs/VPS_FOUNDATION_HANDOFF.md`. Tell the owner the exact commit hash from which the three desktop worktrees must start. After that checkpoint, continue the VPS implementation in parallel with the other chats. Do not keep redesigning Contract v1 underneath them; changes require a versioned ADR and compatibility plan.

## Docker-first development and VPS delivery

Local developers must not need to install PostgreSQL, Redis, Nginx, or NestJS globally. Node and package-manager versions are pinned in the repository; application commands run through repository scripts or containers.

Deliver:

- A multi-stage, non-root production Dockerfile for the NestJS API/worker.
- `compose.dev.yml` for local API + PostgreSQL development with health checks and named volumes.
- `compose.prod.yml` for VPS runtime with restart policies, private networks, persistent volumes, resource/logging limits, and no public PostgreSQL port.
- An explicit one-shot migration service/job. Do not let every API replica race migrations on startup.
- `.env.example` containing names and safe documentation only—never secrets.
- Health and readiness endpoints that distinguish process health from database/migration readiness.
- `scripts/dev-up`, `dev-down`, `test`, `backup`, `restore-test`, `deploy-vps`, and `rollback` equivalents for Windows-friendly local use and Linux VPS use.
- A GitHub Actions pipeline that lints/tests, builds immutable images, scans them, and publishes approved images to GitHub Container Registry.
- A production deployment path where the VPS pulls the repository/configuration and immutable image, then runs Docker Compose. The normal release must not require manually installing Node or PostgreSQL on the VPS.
- A documented first-time VPS bootstrap for Docker Engine/Compose, firewall, deployment user, directories, secrets, HTTPS, and backup destination. Be explicit that first-time server and DNS configuration cannot be eliminated by Docker.
- A routine release command equivalent to `git pull --ff-only` followed by migration and `docker compose pull && docker compose up -d`, wrapped in a checked deployment script with health verification and rollback.

For the first local milestone, `docker compose -f compose.dev.yml up --build` must start the backend and PostgreSQL from a clean machine with documented prerequisites. Do not claim that Avalonia GUIs run in this Linux compose stack.

## Your module: VPS, API, admin website and release hosting

You own the central backend and admin UI, shared API/event contracts, central schema and deployment/release endpoints. You do not own local stock writes. Coordinate contract changes through versioned schemas, fixtures and ADRs; do not change desktop semantics unilaterally.

### Work packages

1. Implement PostgreSQL migrations for the complete central model. Mirror site events into normalized operational tables and ledger projections; preserve original IDs, immutable facts, posting dates and actor/device metadata. Apply tenant/site authorization in every query path, including exports/downloads. Design read models for branch stock, retail revenue, cafe receivables, kitchen ingredients, unresolved conflicts and sync health. Avoid exposing private audit columns through generic event feeds.
2. Build enrollment: site creation with type, one-time code issuance/expiry/use, profile-bound device credential, writer uniqueness, device revocation and recovery procedure. A Type 1 device cannot invoke Type 2 cafe APIs even if it modifies its UI or request body.
3. Build sync ingestion/dedup and permission-filtered cursor feed; immutable snapshots for bootstrap and resync. Test realistic out-of-order dependencies from different sites: kitchen cannot dispatch an unknown branch request; branch cannot receive an unknown shipment. Return precise retryable dependency or permanent rejection codes. Store event acceptance and resulting projection transactionally.
4. Implement workflow services for request routing, shipment read models, conflicts and admin decisions. Decision screen compares requested/sent/counted quantities, relevant timestamps, notes, original receipt and site freshness; requires final per-line count and reason. Decision is not finished until the site confirms application. Reject double decision/stale version. Never grant branch/kitchen users resolution permission.
5. Implement server-authoritative cafe pricing and centralized payment allocation. UI: customer list with searchable name, unpaid amount/status text, shared base list assignment, explicit per-customer overrides, past invoices, invoice detail, partial payment and remaining balance. An unpaid red indicator must also have a text label. No general accounting ledger or tax-compliance claim is in scope.
6. Build report artifact upload/download with access checks, allowed size/type, content hash verification and version immutability. Stream files; no user-controlled filesystem paths. Surface XLSX missing/failed separately from a successfully closed shift. Download lists show branch, business date, shift and report version. Do not parse Excel back into stock.
7. Implement low-ingredient/variance, unresolved dispute and stale-device alerts. Deduplicate threshold alerts until recovery; acknowledgement is not replenishment. Display recorded waste versus unexplained count variance and chosen cost basis.
8. Build named-user security, role administration, admin-only correction audit, server audit retention, sessions/recovery and observability. Break-glass access must be explicit and audited, not a secret universal password. Protect signing keys and enrollment secrets.
9. Package API, worker, web and database orchestration for local development and single VPS. Include reverse-proxy configuration, HTTPS setup instructions, health endpoints, firewall expectations, volumes, offsite backup configuration and restore runbook. Migrations run as an explicit release job, not concurrently on every replica.
10. Host profile-specific signed installers and update manifests on this same VPS. Backend validates release profile/channel/compatibility. Installer builds execute on Windows CI; artifacts are promoted only after their test gate. Prepare repository CI and deploy scripts, but ask for confirmed repository name/account before remote creation or push and approval before live deployment.

### Minimum API surface

Publish OpenAPI plus event schemas and example payloads for: login/refresh/logout; users/roles; sites; enrollment tokens and enroll/revoke; scoped bootstrap; sync push/pull/ack; kitchen requests and shipments read views; conflict list/detail/decision; adjustment approval; catalog/retail price/recipe publication; customers and price-list revisions/overrides; invoice lookup, payment creation/allocation/reversal; stock/revenue/history views; report upload/download; releases and update manifests; health/readiness. Operational commands from desktops travel through the same idempotent event/command service, whether exposed as typed endpoints or sync messages. Do not create two independent posting paths.

Error contract: stable code, safe message, correlation_id, field errors, retryability, current_version when relevant; HTTP 401/403 for identity/scope, 409 stale state, 422 business validation, 429 limits, and retryable 5xx as appropriate. Use pagination and maximum batch/upload sizes. Never return secret hashes or raw stack traces.

### Admin navigation and reporting acceptance

Dashboard -> branch -> stock/retail revenue -> month -> day -> morning/evening -> shift -> receipt. Separate cafe area -> customer -> invoiced/collected/outstanding -> invoice -> allocations. Kitchen area -> ingredients -> count/waste/variance -> recipe consumption source. Conflicts -> decision -> pending site apply -> resolved. Operations -> device status, reports, releases/backups. Admin can inspect both aggregate and source documents; sums match ledger and payment facts.

Example acceptance: retail Branch A 1,200,000 and Branch B 100,000; cafe invoices 800,000 and collected 500,000 => cafe outstanding 300,000, not another 500,000 revenue. Every amount labels its date range, basis, currency and sync freshness. Sales reported by issuing site are distinct from collections by collecting site. No built-in profit total without costs/expenses coverage.

### Migration and rollout

Import legacy local data through versioned migration/export mapping, not copying local integer IDs into shared PKs. Use deterministic mapping by site + legacy table + old ID; preserve original numbers/timestamps/snapshots and import reconciliation totals. Seed the new continuous ledger from one verified cutover balance; archive/import old detailed history without also replaying its effects into that balance. Never import plaintext security answers as active authentication. Existing Data folders are sensitive and never enter Git or release artifacts. Pilot one branch with synthetic then owner-approved cutover data; observe outbox health and report reconciliation before remaining devices.

### Completion evidence

Run integration tests against real PostgreSQL, authorization tests for every role and branch type, offline/retry contract tests with SQLite clients, duplicate payments, late decisions, file access and hash checks, schema upgrade/downgrade-compatibility cases and backup restore. Deliver deployment dry-run output and staging health/smoke evidence. State clearly if VPS access, DNS, signing certificate or Windows runner was unavailable. Production release remains gated until these are supplied and verified.
