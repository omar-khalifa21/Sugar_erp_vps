# Implementation agent prompt: Branch Type 2
...
**First action: verify that `docs/contracts/CONTRACT_V1.md` and the VPS foundation handoff exist. If not, stop and request the Contract v1 checkpoint. Work only in a dedicated Branch Type 2 worktree created from that checkpoint.**

Do not redesign server contracts or central schemas. Generate the C# client/models from the frozen contract where practical, and request changes through an ADR. The application runs natively on Windows; SQLite is embedded and requires no external database installation. Provide reproducible .NET restore/build/test commands and a Windows installer pipeline. Finish with `docs/handoffs/BRANCH_TYPE_2_HANDOFF.md`.

## Your module: Branch Type 2 desktop

Implement the retail branch supplied every two or three days, with FREEZER and DISPLAY locations and cafe credit orders. Reuse Type 1's tested retail/shift/sync/security components; expose additional location and cafe capabilities only for this profile. Do not hardcode a single branch ID or fork all POS logic.

### Local schema extensions

Use all common branch tables plus locations for FREEZER/DISPLAY and hold tracking, internal transfer headers/lines linked to stock_documents, customers, price-list versions/items/assignments/overrides, external invoices/lines, payments/allocation cache and pending collection state. Add appropriate item/location/shift indexes; snapshot each location, not one undifferentiated stock number. Never join external invoice values to today's price table for historical display.

### Location policies

Accepted kitchen receipts go to FREEZER. A FREEZER -> DISPLAY transfer subtracts/adds equal quantity atomically. Retail sales consume DISPLAY only. Cafe invoices consume FREEZER only. Kitchen returns consume FREEZER; to return displayed goods first record DISPLAY -> FREEZER movement so history matches reality. Close records unsold display quantity then returns it to FREEZER in one close transaction before final per-location snapshot. If physical display count differs, preserve discrepancy and require the count-adjustment approval policy; do not transfer imaginary goods or erase the mismatch. Under default policy, unresolved display discrepancy blocks final return/close until reconciled or an explicit approved exception is recorded.

Do not auto-transfer or borrow freezer stock because display is short. Show a transfer action requiring confirmation. If freezer has 50 and display has 0, a retail sale must fail until display transfer; a cafe dispatch can use freezer. Do not call an internal transfer وارد from kitchen or a sale. No ingredient deductions on branch receipts/sales; kitchen already owns consumption.

### Screens and workflows

1. Profile-specific installer/enrollment, staff login, offline recovery and catalog/printer/export setup.
2. Overview shows freezer, display, held/in-transit and total separately; search and physical counts by location. Other branch stock is read-only/stale-aware.
3. Kitchen requests and counted receipts use the shared state machine, multiple shipments and admin conflict pathway. Incoming is not saleable/display until accepted and transferred.
4. Transfer screen: source/destination, searchable items, available quantity, transfer quantity, reason, user/time, immutable transfer history. Duplicate taps/retries cannot double transfer.
5. POS/history/refunds as Type 1 but source DISPLAY. Returned retail food only restores display if explicitly saleable; default physical food return goes to hold/discard per approved policy. Incorrect-entry reversal restores original location.
6. Cafe tab: searchable cafe list, balance/overdue label, new order, past digital invoices and authorized pricing workflow. Pick cached customer/base list; apply override precedence; snapshot effective prices. Show credit amount before issue. Issue reduces FREEZER and queues print. Mandatory print attempt must be retried visibly if printer fails; do not issue a second invoice as a workaround.
7. Pricing: choose existing shared base list when adding customer through authorized online flow. Independent cafe change writes override or deliberate clone, not a global list edit. Offline new customer/pricing requests remain pending, not globally approved records. Invoice known customers with cached published versions offline; show revision and stale warning.
8. Collections: invoice detail shows original amount, prior allocations, outstanding and a Record payment action. Support full/partial amounts and method/date/reference. Online central allocation finalizes against latest outstanding; offline collection stays visibly pending/unallocated. Prevent multiple staff/sites turning the same invoice paid twice. Collections affect cash drawer but do not create another stock issue or revenue event.
9. Close: review kitchen returns and cafe issues, count freezer/display, resolve transfer/count discrepancy, atomically return display balance to freezer and freeze snapshots/cash. Report display unsold-before-return and final display separately. Cafe invoiced versus cafe collected totals stay separate from retail.
10. History: all shared shift/history features plus location transfer drill-down, cafe invoices/payments and stock source. All selectors searchable; no keyboard-only required action. Staff cannot view private correction audit.

### Required conservation example

Start freezer 100/display 0. Transfer 30 => 70/30. Retail sale 20 => 70/10. Cafe issue 15 => 55/10. Close returns display 10 => freezer 65/display 0, total 65. Report retail sold 20, cafe dispatched 15 and internal transfer totals; no transfer revenue. Next shift freezer opening 65/display 0; staff transfers new display quantity explicitly. Replaying all sync events must preserve 65, not double deductions.

### Additional acceptance tests

- Concurrent retail sale and close transfer serialize; only one consistent outcome, no negative display or untracked sale.
- Crash between transfer legs rolls back both legs and balance/outbox; total stock conserved.
- Cafe A and B share base price 70; A override 65 => B remains 70. Older A invoice at 70 still prints 70.
- Invoice 80,000; payment 30,000 => outstanding 50,000, another 50,000 => paid, not a third revenue posting. Concurrent over-allocation from kitchen is rejected.
- Duplicate invoice submission prints/retries the original; freezer deduction once. Cafe return endpoint is absent/rejected.
- Physical display 8 while ledger 10 at close => discrepancy -2 remains recorded; cannot silently move 10 actual goods back to freezer. Exercise approved adjustment then close and report both events.
- Kitchen receipt conflict resolution on next shift posts to FREEZER then; prior close/report stays immutable.

### Deliverables

Real reusable profile code, SQLite migrations/indexes, transfer/cafe domain and integration tests, API contract fixtures, headless touch/RTL screens, printer/Excel fixtures, clean Windows installer/update evidence, operator instructions, recovery runbook and exact remaining blockers. Keep shared contract unchanged unless a reviewed decision updates all module copies. Follow repo-name question and remote/deployment authorization gates.
