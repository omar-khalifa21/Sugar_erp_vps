# Implementation agent prompt: Kitchen
...
**First action: verify that `docs/contracts/CONTRACT_V1.md` and the VPS foundation handoff exist. If not, stop and request the Contract v1 checkpoint. Work only in a dedicated Kitchen worktree created from that checkpoint.**
...
## Your module: Kitchen desktop

Build the kitchen Windows/Avalonia application with its own SQLite database, staff authentication, offline operation after enrollment, ingredient inventory, branch request/dispatch workflow, returns handling and cafe orders. Kitchen is NOT the central server. It installs from the same VPS as the two branch installers. No machine-specific hardcoded branch or remote DB credentials.

### Local schema and stock model

Use common foundation, ingredients/products catalog, recipes/versions/lines, ingredient receipts/costs, ingredient ledger/balances, consumption batches/lines, counts/variance/adjustments, thresholds/alerts, downloaded branch requests, shipments/lines, return receipts/lines and holds/dispositions, cafe pricing/invoices/payments, operator sessions/shifts, audit, outbox/inbox/cursors and print/report jobs. Product quantities in dispatch/returns are separate from ingredient base-unit quantities.

Default dispatch-derived consumption does not claim to track physical finished-goods production inventory. Record returned finished goods in a distinct HOLD/RETURNED_PRODUCTS location pending disposition. Default discard/hold; do not reuse them as raw ingredients. If user wants resale of returned finished goods or pre-made kitchen stock, stop that feature and design the approved finished-goods/production mode before implementation, including avoiding repeat ingredient deduction. The dispatch-only baseline is an explicit approximation requested by the user, not a hidden full manufacturing model.

### Work packages

1. Installer/enrollment chooses KITCHEN profile and verifies registered kitchen site. Setup includes printer, export path, authorized users, ingredient catalog and recipe snapshot download. No kitchen PC database exposure to the internet.
2. Ingredient stock screen: base units, receipts, expected on-hand, low threshold, last count, recorded waste, pending variance and costing. Receipts record received quantity, reference, time, actor and cost; validate unit conversions. Stock receipt/edit permission is separate from branch shipment permission.
3. Recipe display: output product, yield, ingredient quantities, effective version and usage example. Publishing new recipe/threshold follows authorized central workflow; offline edits are proposals unless explicitly assigned kitchen authority. Dispatch uses exact cached approved version; no recipe => fail with clear error, not zero consumption.
4. Request queue: branch name/type, request number/time, item requested/approved quantities and sync freshness. Accept all, reduce selected quantities, or reject with reason. Do not add unrelated items or dispatch more than requested without revised branch approval. Support partial shipments with remaining requested balance. Queue response while offline and label not yet delivered.
5. Dispatch preview: branch destination, immutable shipment number, output lines, recipe versions and ingredient requirements, stock feasibility, timestamp. On confirmation commit shipment, consumption_batch UNIQUE dispatch_id, per-ingredient deductions, cost snapshots, audit, outbox and print job together. Duplicate confirm is idempotent. No deduction on draft/approval alone; no repeat deduction when branch accepts or admin resolves.
6. Shipment history: dispatched quantities, receipt counted values and conflict/decision state. Kitchen cannot change sent quantities after dispatch or resolve disputes. An actual post-dispatch correction is a new audited workflow with transit implications, not an edit to the original.
7. Branch returns: inspect expected sent quantity, record received quantity and timestamp, match/raise admin conflict, place confirmed goods in hold with disposition. Product return never adds eggs/flour/butter. Discard records waste in finished-product units; do not count it again as fresh ingredient consumption.
8. Cafe orders: share exact price-list/customer/payment semantics with Type 2, issue digital and printed invoice, expected receivable, payments and outstanding. Kitchen issue triggers the same dispatch-derived ingredient consumption once, linked to external invoice fulfillment, not both invoice save and print. Cafe returns are unavailable. Offline payment collection is pending until central allocation.
9. Physical count and variance: let user report actual exhausted stock even when computed stock says quantity remains; record immutable count, flag shortage, alert admin, request adjustment. Do not invent extra receipt or silently go negative. Approved correction applies as separate movement through local writer. Monthly report separates recipe expected usage, logged waste, count-based unexplained loss, opening/closing and valuation.
10. Threshold alerts: per ingredient editable by authorized role, low/recovery levels and dedup; push admin alert via sync and show local warning offline. Replenishment resolves threshold incident; acknowledgement alone does not.
11. History and operator close: searchable dispatch/cafe/receipt/count records, dates/timestamps, branch/customer filters, print/reprint, shift/day summaries and Excel (.xlsx) uploads where enabled. Confirm kitchen morning/evening session use with owner; cash collections must always have operator/time and reconciliation session if accepting money.

### Atomic examples and acceptance

- Recipe cake v1 yield 1 cake, eggs 5: 20 cakes to branch => eggs -100 at dispatch. Request approval, branch acceptance, conflict decision, sync replay and reprint add no further deduction.
- Same recipe: cafe issue 4 cakes => eggs -20 once; cafe payment changes receivables/cash only.
- Recipe changes from 5 to 6 eggs after shipment: old batch remains 100 eggs; new 20-cake dispatch requires 120.
- Ingredient receipt 1,000 eggs; dispatch consumption 300 => expected 700. Count actual 200 => unexplained shortage 500; admin-approved -500 adjustment yields ledger 200. Monthly report still reports shortage 500 after adjustment, not zero or 1,000.
- Return 3 cakes from branch => 3 returned finished goods held, eggs unchanged. Neither branch return acknowledgement nor disposal reconstructs ingredients.
- Two pending branch dispatches need 80 eggs each and stock is 100: serialize; only one posts, other reports shortage. UI's stale balance cannot overspend.
- Missing recipe, mismatched unit or expired authorization blocks posting with actionable message; no partly saved consumption.
- VPS disconnect and restart retains outbox, sent documents, print failures and ingredient balance. Invalid incoming decision cannot corrupt local stock.

### Delivery and operational gates

Deliver migrations, exact units/yield calculation tests, cost/variance reconciliation fixtures, branch and cafe end-to-end tests, permission tests proving kitchen cannot resolve conflict, Arabic touchscreen screens, Windows printer proof, kitchen installer and upgrade test with unsynced dispatch. Provide ingredient count/receipt/dispatch/return/threshold operator guide and backup/restore runbook. Follow shared first question for project/repository name and account; develop locally, push only to confirmed remote with permission, deploy only after owner-approved staging gate.
